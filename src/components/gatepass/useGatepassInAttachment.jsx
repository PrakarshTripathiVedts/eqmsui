...
import { downloadGatepassInFile } from "../../services/gatepass.service";
 ...
 */
const useGatepassInAttachment = () => {
  const [loading, setLoading] = useState(false);

  const openGatepassInAttachment = async (gatepassId, filename) => {
    if (!gatepassId) return;
    setLoading(true);
    try {
      const { blob, contentType } = await downloadGatepassInFile(gatepassId);
  ...
  return { openGatepassInAttachment, loading };
};

export default useGatepassInAttachment;