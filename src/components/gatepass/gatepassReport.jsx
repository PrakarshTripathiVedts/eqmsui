...
const GatepassReport = () => {
  ...
  const tableRows = filteredData.map((item, idx) => [
    ...
    item.category           ?? "-",
    projectList.find(p => p.projectId === item.projectId)?.projectName ?? "-",
    item.destination        ?? "-",
          ...
          <div className="mb-3 p-3 rounded" style={{ backgroundColor: "#f0f4ff", border: "1px solid #c7d4f0" }}>
            ...
            <div className="col-md-3 text-start" style={{ zIndex: 10, position: "relative" }}>
                ...
                <Select
                    ...
                    isClearable={false}
                    menuPortalTarget={document.body}          // ← renders dropdown outside DOM flow
                    styles={{ menuPortal: base => ({ ...base, zIndex: 9999 }) }}  // ← always on top
                />
            </div>
...
<div className="table-responsive border rounded" style={{ maxHeight: "500px", overflowY: "auto" }}>
  <table className="table table-sm table-bordered table-hover mb-0 text-center align-middle" style={{ fontSize: "0.83rem" }}>
        ...
        filteredData.map((item, i) => {
          ...
          return (
            <tr key={i} style={{ backgroundColor: i % 2 === 0 ? "#fff" : "#f0f4ff" }}>
              {row.map((cell, j) => (
                <td key={j} className={j === 4 || j === 5 ? "text-start" : ""}>{cell}</td>
              ))}
